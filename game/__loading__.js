...
pc.script.createLoadingScreen(function (app) {
    ...
    var doAPIHandshake = function(startGameCallback) {   
        ...
        if(isExternalStart()) {
            ...
            /* Embedded shell never calls host startGame — unstick after load */
            setTimeout(function() {
                if(app.timeScale === 0 && typeof famobi.requestAction === "function") {
    ...
    var startLevelDirectly = function() {
        if (typeof MissionsManager === 'undefined' || typeof EventTypes === 'undefined') {
            console.warn('Airtel: cannot start level — game scripts missing');
            return;
        }
    ...
    var forceFinishLoad = function() {
        var splash = document.getElementById('application-splash-wrapper');
        if (!splash) return;
        famobi.log('Airtel: force finishing load screen');
        try { stopPreloaderSimulation(); } catch(e) {}
        if (typeof displayProgress === 'function') displayProgress(1);
        else setVisibleProgress(1);
        hidePreloader();
        try {
        ...
        if (!isAirtelEmbed()) {
            try {
                if (typeof famobi.requestAction === 'function') famobi.requestAction('startGame');
            } catch(e) {}
            try { if (app && app.timeScale === 0) app.timeScale = 1; } catch(e) {}
        }
    };
    setTimeout(forceFinishLoad, 10000);
    ...
    var finishBoot = function() {
        ...
        if (typeof isForcedMode === 'function' && isForcedMode()) {
        ...
        if (isAirtelEmbed()) {
            try { app.timeScale = 0; } catch (e) {}
            famobi.onRequest('startGame', function () {
                try { app.timeScale = 1; } catch (e) {}
            });
        } else if (typeof skipTitleScreen === 'function' && skipTitleScreen()) {
            startLevelDirectly();
        } else {
            doAPIHandshake(() => {
                if (typeof EventTypes !== 'undefined') {
    ...
    app.on('postinitialize', () => {
        setTimeout(() => {
            if (typeof EventTypes === 'undefined' || typeof AssetsLoader === 'undefined') {
                console.warn('Airtel: game scripts not ready — use: python3 serve.py');
                finishBoot();
                return;
            }
            ...
            AssetsLoader.getInstance().loadPendingAssets().then(() => {
                ...
                console.error('Airtel: pending assets failed', err);
                finishBoot();
            });
        }, 50);
    });
